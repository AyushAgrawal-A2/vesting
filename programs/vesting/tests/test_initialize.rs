...
fn test_initialize() {
    ...
    let counter = Pubkey::find_program_address(
        &[vesting::constants::COUNTER_SEED],
        &program_id,
    )
    .0;
    ...
    let instruction = Instruction::new_with_bytes(
        program_id,
        &vesting::instruction::Initialize {}.data(),
        vesting::accounts::Initialize {
            payer: payer.pubkey(),
            counter,
    ...
    let counter_state = vesting::state::Counter::try_deserialize(&mut data).unwrap();
    assert_eq!(counter_state.count, 0);
    assert_eq!(counter_state.authority, payer.pubkey());

    let instruction = Instruction::new_with_bytes(
        program_id,
        &vesting::instruction::Increment {}.data(),
        vesting::accounts::Increment {
            counter,
